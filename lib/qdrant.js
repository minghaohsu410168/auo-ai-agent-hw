...
export const LANDMARK_COLLECTION = "landmarks";
...
export async function searchLandmarks(query, limit = 5) {
  const vector = await embed(query);

  const results = await qdrant.search(LANDMARK_COLLECTION, {
    vector,
    limit,
    with_payload: true,
  });
  ...
  return results.map((r) => ({
    ...
    best_season: r.payload.best_season,
    description: r.payload.description,
  }));
}
